...
Webhook: payment_intent.succeeded ──► prisma.$transaction() ──►
         ...
         ├── Wallet { pendingBalance += developerPayout } (via fulfillOrder)
         └── Wallet { pendingBalance -= developerPayout, availableBalance += developerPayout } (via deliverOrder)
...
// 5. Update developer Wallet (Payment Fulfillment Phase)
Wallet.update({
  pendingBalance: { increment: developerPayout }
})

// Note: During the Asset Delivery phase, a separate transaction will run:
// Wallet.update({
...
```js
const orderIds = (
  await prisma.order.findMany({ where: { clientId: clientId } })
).map((o) => o.id);
...
await prisma.wallet.updateMany({
  where: { userId: devId },
  data: { availableBalance: 0, totalEarnings: 0 },
});
...
- [ ] `Wallet.pendingBalance` increased by exactly `developerPayout` upon payment
- [ ] `Wallet.pendingBalance` decreases and `Wallet.availableBalance` increases upon delivery
