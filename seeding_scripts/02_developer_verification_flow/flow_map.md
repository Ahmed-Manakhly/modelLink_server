...
await prisma.developerVerification.deleteMany({ where: { userId: { in: devIds } } });
await prisma.wallet.deleteMany({ where: { userId: { in: devIds } } });
// Also clear notifications
await prisma.notification.deleteMany({ where: { recipientId: { in: devIds } } });
// Reset isVerified flag
await prisma.user.updateMany({ where: { id: { in: devIds } }, data: { isVerified: false } });
```
